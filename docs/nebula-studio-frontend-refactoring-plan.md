...
- [ ] 用 `PersonalWorkspace` 替换 Shell 的默认“暂无内容”状态。
- [ ] 汇总最近访问、我的申请、待办、运行异常、常用资源和快捷创建。
- [ ] 将应用坞改为应用管理/启动器，支持按角色、分类和最近使用筛选。
- [ ] 建立统一页面元数据：标题、图标、help key、搜索关键词、所需角色和返回目标。
- [ ] Web 与 Electron 使用相同工作台数据模型，在窄窗口降级为单列任务流。

#### Phase 3B：全局导航与反馈

- [ ] 增加全局搜索/命令面板，首期覆盖应用、资源、文档和页面动作。
- [ ] 将通知中心升级为可执行待办，支持定位到资源、申请、审批或错误详情。
- [ ] 统一子应用 loading、加载失败、离线、会话过期和权限变化的 Shell 级恢复界面。
- [ ] 优化侧栏、标签页、面包屑和返回行为，避免 Shell 与子应用出现双重导航。
- [ ] 保留 iframe 隔离，但建立页面标题、焦点恢复和跨应用跳转协议。

#### Phase 3C：Login/AuthFlow

- [ ] 将 Login 重构为显式步骤状态机：账号、组织、MFA、恢复、成功和失败。
- [ ] 增加可搜索组织选择、最近组织、返回登录和会话失效上下文。
- [ ] Demo 账号只在开发配置开启时展示。
- [ ] 为账号锁定、凭证错误、服务不可用、网络离线和未知错误提供不同恢复动作。
- [ ] 保持 Web/Electron 品牌与布局连续性，并验证自动填充、密码管理器和键盘提交。
...
- [ ] 建立跨 API、库表、Connector 的 `ResourceSummaryViewModel`。
- [ ] 实现门户首页：全局搜索、资源类型、常用分类、最近访问和申请进度摘要。
- [ ] 实现资源目录：关键词、类型、标签、提供方、可申请状态、排序和分页。
- [ ] 筛选条件同步 URL，支持分享、刷新恢复和浏览器前进/后退。
- [ ] 用真实目录 API 替换当前插件市场的本地模拟搜索。

#### Phase 4B：可理解、可申请

- [ ] 建立统一资源详情页，展示摘要、用途、负责人、版本、SLA、权限范围和兼容性。
- [ ] 按资源类型插入 API 示例、库表字段、Connector 配置等领域详情，不复制详情页壳。
- [ ] 实现分步申请向导：用途、环境、期限、权限范围、数据敏感性确认和提交预览。
- [ ] 实现我的申请与状态时间线，明确待补充、审批中、通过、拒绝、到期等下一步动作。
- [ ] 失败保留表单草稿；重复申请、无权限和资源下线均提供可行动的错误说明。

#### Phase 4C：可使用

- [ ] 将“我的服务”和“库表订阅”收敛为“我的资源/我的订阅”。
- [ ] 为已获资源提供接入地址、鉴权方式、示例、凭证状态、订阅事件和续期入口。
- [ ] 首期支持收藏、最近访问和最近申请；推荐算法不作为上线前置条件。
- [ ] 埋点覆盖搜索、查看详情、开始申请、提交、获批后首次访问等关键漏斗。
